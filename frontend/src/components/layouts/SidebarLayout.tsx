...
export const SidebarLayout: React.FC<SidebarLayoutProps> = ({
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-slate-50 text-slate-800 font-sans">
      ...
      {/* 2. 반응형 사이드바 (밝고 깨끗한 화이트 테마) */}
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${
        ...
        <div className="flex h-16 items-center justify-between border-b border-slate-100 px-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold tracking-wider bg-gradient-to-r from-sky-600 to-indigo-600 bg-clip-text text-transparent">
              AI-PILOT
            </span>
          </div>
          {/* 모바일 닫기 버튼 */}
      ...
      <main className="flex flex-1 flex-col overflow-hidden bg-slate-50">
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 md:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold tracking-wider text-slate-800">AI-PILOT</span>
          </div>
